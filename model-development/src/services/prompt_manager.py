...
class PromptManager:
    ...
    def __init__(self):
        ...
        missing_prompts = [
            ...
            if pid[0] not in available_prompt_ids
        ]
        if missing_prompts:
            raise ValueError(
    ...
    def get_prompt(self, prompt_name: str) -> str:
        prompt_info = settings.prompt_setting.prompt_mapping.get(prompt_name)
        if not prompt_info or not (prompt_info[0] or prompt_info[1]):
            raise ValueError(f"No prompt id configured for prompt '{prompt_name}'")
        prompt_id, prompt_version = prompt_info
        prompt = prompts.get(prompt_id=prompt_id, version_id=prompt_version)
